...
class B2C_trade_wechat:


    def B2C_trade_PC_wechat( result_orderinfo,out_trade_no,url_dict):
        #重定向
        session = requests.Session()
        m_url="https://cashier.mideaepayuat.com/autologin.htm?order="
        ...
        send_data["tradeNo"]=trade_no

        wechat_url = "https://cashier.mideaepayuat.com/eBank.json"
        r=session.post(wechat_url, send_data, verify=False)
        return r.json()
        ...
        # # 触发定时任务
        # elasticjob(url_dict, "escrow-channel-job").jobTrigger("DeductDetailOrderRegularRepairJob")
