...
class B2C_trade_ebank_pay(object):
    def __init__(self,url_dict):
        self.url = url_dict
        self.in_headers = {'referer': self.url["B2C_trade_url"]}

    def B2C_trade_PC_ebank_pay(self, pay_data, order_info):
        ...
        #
        # r = requests.post(self.url["Pay_ebank_url"], getgenPwd_data, verify=False)
        # r = r.json()
        # data = r['data']
        ...
        #==================================================================================
        r = session.post(self.url["Pay_ebank_url"], paydata, headers=self.in_headers,verify=False)
        #result = r.json()
        logger.info(r)
        #退出登录
